...
def plot_normal_pdf(mu=0, sigma=1, lbl="Sample"):
    """plot function normal_pdf within +- 5 Z"""
    xs = [(x/10)+mu for x in range(-50*sigma, 50*sigma)]
    max_value = max([normal_pdf(x, mu, sigma) for x in xs])
    plt.plot(xs,[normal_pdf(x, mu, sigma) for x in xs],
             label=('{}: {} +/-{}'.format(lbl, mu, sigma)))
    plt.ylim(ymin=0, ymax=(1.5 * max_value))
    plt.legend()
    return
...
def plot_binomial_exp(p, n, num_points):
    ...
    data = [binomial_exp(p, n) for _ in range(num_points)]
...
# Visualizations
###############################################################################
#TODO: paralell coordinates, density plot
...
def make_scatterplot_matrix(data):
    ...
    for i in range(num_columns):
        for j in range(num_columns):
            ...
            if i != j: ax[i][j].scatter(get_column(data, j),
                                        get_column(data, i))
...
def plot_bucket_histogram(points, bucket_size, title=None):
    histogram = bucketize_points(points, bucket_size)
...
def plot_14c_pdfs(date_dict, plot_type='plot', title_=None):
    """Plot 14c - as single plot or stacked subplots
        date_dict = {key (sample): list[date, sterr]}

        i.e. date_dict - {'sample 1': [250, 35],
                          'sample_2': [300, 50]}
    """

    # Highest at the peak
    y_upper = max(normal_pdf(date[0], date[0], date[1])
                    for date in date_dict.values())

    x_lower = min([date[0]-5*date[1] for date in date_dict.values()])

    x_upper = max([date[0]+5*date[1] for date in date_dict.values()])

    if plot_type.lower() == 'plot':
        for sample, c14 in sorted(date_dict.items(),
                                  key=operator.itemgetter(1)):

            plot_normal_pdf(c14[0], c14[1], sample, x_lower, x_upper)
